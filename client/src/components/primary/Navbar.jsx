...
export default function Navbar() {
    ...
    return (
        <>
            <div className='hidden lg:flex bg-orange-100  justify-center flex-col items-center relative '>
                <div className='flex w-full justify-center align-center gap-12 text-sm py-2'>
                    {Object.keys(navItems).map((item, index) => {
                        return <p key={index} onMouseOver={() => { setShowNavDropdown(true); setSelectedNav(item) }} className={`hover:text-orange-600 ${selectedNav=== item && showNavDropdown ? 'text-orange-600' : 'text-black'} cursor-pointer flex gap-2 text-sm py-2 items-center`}>{item}</p>
                    })}  

                </div>
                <ul className={`dropdown w-full shadow-s z-400 shadow-gray-100 max-h-40 absolute top-12 py-6 px-8 text-xs text-gray-600 bg-white ${showNavDropdown ? 'flex' : 'hidden'} flex-col flex-wrap align-middle gap-2`} onMouseOver={() => setShowNavDropdown(true)} onMouseOut={() => setShowNavDropdown(false)}>
